...
def query_stock_graph(x):
    #driver = webdriver.Chrome('C:/Users/Krushna/Downloads/chromedriver.exe')
    my_stock = x.get()
    print(my_stock)
    try:
...
def query_stock_info(x):
    ...
    para = my_stock + ".NS"
    '''try:
        url = 'https://www.screener.in/company/' + my_stock + '/consolidated'
        response = requests.get(url)
    ...
    market_cap = get_quote_table(para)['Market Cap']
    current_price = get_quote_table(para)['Quote Price']

    range = get_quote_table(para)['52 Week Range'] 
    low_str, high_str = map(lambda x: x.replace(',', ''), map(str.strip, range.split('-')))
    # Convert the strings to float values
    year_low = float(low_str)
    year_high = float(high_str)

    mar_cap = 'The market capitalization of ' + my_stock + ' is ' + market_cap
    price = 'The current price of ' + my_stock + ' is ' + str(current_price)
    high = 'The 52 week high price of ' + my_stock + ' is ' + str(year_high)
    low = 'The 52 week low price of ' + my_stock + ' is ' + str(year_low)
...
def main(user):
    ...
    root.title('Kuber Online Trading Platform')
    # print(user)
    ...
    welcome_text = 'Welcome dear ' + user + ' ,' + 'We hope that you would enjoy using Kuber.'
